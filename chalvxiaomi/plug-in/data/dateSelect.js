...
$.fn.dateSelect = function(options) {
	...
	var _dateBody = "<div class='tz-datepicker' id='datepicker_"+elem_id+"'>" +
			"<div class='year-month' id='ym"+elem_id+"'><span class='left'><img src='iamge/left-default.png'></span>" +
			...
			"<span class='year'><span class='span-year'>2015</span><div class='year-list' id='ylist_"+elem_id+"'><ul></ul><div class='year-change'><span class='year-left'></span><span class='year-right'></span></div></div></span><span class='right'><img src='iamge/right-default.png'></span></div>" +
    ...
    {	
    	...
    	var mn = newdate.getMonth()+1;
    	var d = newdate.getDate();
    	for(var i=1;i<=days;i++)
    	{
    		var a_str ="";
    		 if(tz_y < y){
    			$("#datepicker_" + elem_id +" .day li").css({color:"#999"}).attr("title","已过期").unbind();
    		}else if(tz_y == y && tz_m < mn){
    			$("#datepicker_" + elem_id +" .day li").css({color:"#999"}).attr("title","已过期").unbind()
    		}else if(tz_y == y && tz_m == mn && d == i){
    			for(var m=0;m<i;m++){
    				a_str = "active";
    			$("#datepicker_" + elem_id +" .day li").eq(m).css({color:"#999"}).attr("title","已过期").unbind();
    			}
    		}else if(tz_y == input_y && tz_m == input_m && tz_d == i)
    		{
    			a_str = "active";
    		}
